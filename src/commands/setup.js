...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const resultsChannel = await findOrCreateTextChannel(guild, CHANNEL_NAMES.resultsChannel, category.id, [
      { id: guild.roles.everyone.id, deny: [PermissionFlagsBits.SendMessages], allow: [PermissionFlagsBits.ViewChannel] },
    ]);

    // Detect the server's real Nitro Booster role (Discord auto-creates/manages this one)
    ...
    saveGuildConfig(guild.id, {
      ...
      booster_role_id: boosterRole?.id ?? null,
    ...
    await interaction.editReply(
      ...
      `- Results log channel: ${resultsChannel}\n` +
      ...
      `- Booster role: ${boosterRole ? boosterRole.name + ' (detected, boosters get queue priority automatically)' : 'Not detected — server has no active boosters yet, but it will still work once someone boosts.'}\n\n` +
